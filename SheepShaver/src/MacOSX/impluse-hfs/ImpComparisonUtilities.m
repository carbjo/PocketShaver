...
ImpBTreeComparisonResult ImpBTreeCompareHFSCatalogKeys(struct HFSCatalogKey const *_Nonnull const quarryCatalogKeyPtr, struct HFSCatalogKey const *_Nonnull const foundCatKeyPtr) {
	...
	NSString *lhsStr = [NSString stringWithCString:(const char*)quarryCatalogKeyPtr->nodeName encoding:NSMacOSRomanStringEncoding];
	NSString *rhsStr = [NSString stringWithCString:(const char*)foundCatKeyPtr->nodeName encoding:NSMacOSRomanStringEncoding];
	NSComparisonResult const nameComparison = [lhsStr compare:rhsStr];
...
NSComparisonResult ImpHFSPlusCompareNames(struct HFSUniStr255 const *_Nonnull const str0, struct HFSUniStr255 const *_Nonnull const str1) {
	...
	for (;;) {
		...
		//Skip ignorable characters on each side.
#if ORIGINAL_IMPLEMENTATION
		for (; idx0 < len0; ++idx0) {
			c0 = CFSwapInt16BigToHost(str0->unicode[idx0]);
			UInt16 const subtableIndex = lowercaseTables[0][c0 >> 8];
			if (subtableIndex != 0) {
				//Found a subtable; fold the character.
				c0 = lowercaseTables[subtableIndex][c0 & 0x00ff];
			}
			if (c0 != 0) {
				break;
			}
		...
		for (; idx1 < len1; ++idx1) {
			c1 = CFSwapInt16BigToHost(str1->unicode[idx1]);
			UInt16 const subtableIndex = lowercaseTables[0][c1 >> 8];
			if (subtableIndex != 0) {
				//Found a subtable; fold the character.
				c1 = lowercaseTables[subtableIndex][c1 & 0x00ff];
			}
			if (c1 != 0) {
				break;
			}
...
#else
		while (len0 > 0 && c0 == 0) {
			c0 = CFSwapInt16BigToHost(*(cur0++));
			--len0;
			UInt16 const tableValue = lowercaseTable[c0 >> 8];
			if (tableValue != 0) {
				//Found a subtable; fold the character.
				c0 = lowercaseTable[tableValue + (c0 & 0x00ff)];
			}
		}
		while (len1 > 0 && c1 == 0) {
			c1 = CFSwapInt16BigToHost(*(cur1++));
			--len1;
			UInt16 const tableValue = lowercaseTable[c1 >> 8];
			if (tableValue != 0) {
				//Found a subtable; fold the character.
				c1 = lowercaseTable[tableValue + (c1 & 0x00ff)];
			}
		}
#endif
		...
		if (c0 == 0) {
			//Both strings are equal so far, and we have reached the end of one string, which means we have reached the end of both.
			return NSOrderedSame;
		}
	}

	return c0 < c1 ? NSOrderedAscending : NSOrderedDescending;
}

#pragma mark Copied from Apple sample code attached to TN1150
 ...
 */
static UInt16 tn1150_gLowerCaseTable[] = {
